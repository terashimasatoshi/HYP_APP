...
`;
    ...
    try {
      ...
      // Geminiは ```json ... ``` でラップして返すことがある
      let jsonContent = content1;
      
      // ```json ... ``` を除去
      const codeBlockMatch = content1.match(/```(?:json)?\s*([\s\S]*?)```/);
      if (codeBlockMatch) {
...
`;
        ...
        // Geminiは ```json ... ``` でラップして返すことがある
        let jsonContent2 = content2;
        const codeBlockMatch2 = content2.match(/```(?:json)?\s*([\s\S]*?)```/);
        if (codeBlockMatch2) {
          jsonContent2 = codeBlockMatch2[1].trim();
        }
        const jsonObjectMatch2 = jsonContent2.match(/\{[\s\S]*\}/);
        if (jsonObjectMatch2) {
          jsonContent2 = jsonObjectMatch2[0];
        }
        
        try {
          const parsed2 = JSON.parse(jsonContent2);
          const r2 = typeof parsed2.report === 'string' ? parsed2.report.trim().replace(/\\n/g, '\n') : '';
          const na2 = typeof parsed2.next_action === 'string' ? parsed2.next_action.trim().replace(/\\n/g, '\n') : '';
          if (r2) report = r2;
