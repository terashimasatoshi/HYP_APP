...
import { TrendingUp, Sparkles, Lightbulb, Home, UserPlus } from 'lucide-react';
...
interface AIReportScreenProps {
  ...
  onHome: () => void;
  /** レポート完了後、そのまま新規顧客登録へ移動したい場合に使用 */
  onNewCustomer?: () => void;
}

type VisitData = {
  before?: { rmssd?: number; sdnn?: number };
  after?: { rmssd?: number; sdnn?: number };
};

export function AIReportScreen({ customerData, onHome, onNewCustomer }: AIReportScreenProps) {
  ...
  // 「戻る」→「進む」でレポート画面を再度開いた場合に、二重登録を避けるための簡易キャッシュ
  // ※ customerData を元にフィンガープリントを作り、同一入力であれば保存・生成をスキップ
  ...
  useEffect(() => {
    const saveAndGenerate = async () => {
      ...
      // 既に同一入力でレポート生成済みなら、二重登録を避けて復元する
      try {
      ...
      try {
        ...
        if (!reportResponse.ok) {
          throw new Error("AIレポートの生成に失敗しました");
        }
  ...
  const improvementRate = customerData.beforeRMSSD > 0
  ...
  const comparisonData = [
    {
      category: 'RMSSD',
      before: customerData.beforeRMSSD || 0,
      after: customerData.afterRMSSD || 0,
    },
    {
      category: '睡眠',
      before: customerData.sleepQuality,
      after: Math.min(10, customerData.sleepQuality + 1),
    },
    {
      category: 'ストレス',
      before: customerData.stress,
      after: Math.max(0, customerData.stress - 2),
    },
    {
      category: '頭皮',
      before: customerData.bodyHeaviness,
      after: Math.max(0, customerData.bodyHeaviness - 3),
    },
  ];
      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600">
            {customerData.name}様の本日の施術効果をまとめました
          </p>
        </div>

        {/* Card 1: Today's Changes */}
        ...
        {currentVisit && (
          <div className="mb-6">
            ...
            <ImprovementChart current={currentVisit} previous={previousVisit} />
          </div>
        )}
        ...
        <div className="space-y-3">
          {onNewCustomer && (
            <button
              ...
              className="w-full py-4 bg-white border-2 border-green-300 text-green-700 rounded-xl shadow-sm hover:shadow-md hover:bg-green-50 transition-all flex items-center justify-center gap-2"
            >
